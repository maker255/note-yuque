...
methods: {
    ...
    handleSuccess(response, file) {
        try {
            ...
            this.fileList.find(item => item.uid === file.uid).url = fileUrl
            this.fileList.find(item => item.uid === file.uid).finalURL = fileUrl
            this.fileList.find(item => item.uid === file.uid).mdURL = `![${file.name}](${fileUrl})`
            this.fileList.find(item => item.uid === file.uid).htmlURL = `<img src="${fileUrl}" alt="${file.name}" width=100% />`
            this.fileList.find(item => item.uid === file.uid).ubbURL = `[img]${fileUrl}[/img]`
