...
// 在 Lake 工厂实例化编辑器之前注入自定义内核/编辑器插件：定制工具栏、注册 insertHTML 命令、
// 为 blockquote / code 注册 HTML 节点读取器。
// 从 ../note-yuque/src/components/lake-rich/editor-plugin.ts 原样移植（纯 TS，无框架耦合）。
...
export function InjectEditorPlugin(
  {
    EditorPlugin,
    KernelPlugin,
    PositionUtil,
  ...
  class CustomKernelPlugin extends KernelPlugin {
    ...
    override init(kernel: any) {
      ...
      if (htmlService) {
        htmlService.registerHTMLNodeReader(["blockquote"], {
        ...
        htmlService.registerHTMLNodeReader(["code"], {
          readNode(context: any, node: any) {
            context.setNode({
              id: node.attrs.id || "",
              type: "element",
  ...
  // 标注未使用，保留以与源实现对齐（如需图片后插入文本时启用）
  void CustomEditorPlugin;
  void ImageInsertAfterCommand;
